...
3. 当某只股票触发买入/卖出信号时，记录信号时间、代码、价格、触发策略，
...
class StockMonitor:
    ...
    def check_once(self):
        ...
        for code in self.watchlist:
            ...
            price = round(float(df_stock['close'].iloc[-1]), 2)  # 当前价（前复权，盘中为实时价）
            context = {'HS300_信号': hs300}

            # ---- 买入信号检测 ----
            if self.use_multi_factor:
                # 多因子模式：逻辑组合通过且评分过门槛才算触发
                result = self._engine.evaluate_stock(code, df_stock, dict(context))
                buy_sign = result.passed
                buy_trigger = result.matched_strategies  # 触发策略=匹配的子策略列表
            else:
                # 单策略模式：沿用原策略2判定
                celue2 = celue_engine.run_single_strategy('策略2', df_stock, context)
                buy_sign = len(celue2) > 0 and bool(celue2.iloc[-1])
                buy_trigger = ['策略2']
            if buy_sign:
                self._alert(code, '买入', price, buy_trigger, today_str)

            # ---- 卖出信号检测 ----
            cache = context.get('_cache', {})
            celue2_series = cache.get('策略2')
            ...
            if len(sell_series) > 0 and bool(sell_series.iloc[-1]):
                self._alert(code, '卖出', price, ['卖策略'], today_str)

        self.logger.info(f'本轮检查完成 用时 {(time.time() - tick):.2f} 秒')

    def _alert(self, code, signal, price, trigger_strategies, today_str):
        """生成预警：同一天同一股票同一信号只预警一次。写预警日志+信号CSV"""
        key = (code, signal, today_str)
        if key in self._alerted:
            return
        self._alerted.add(key)
        trigger = '|'.join(trigger_strategies)
        now = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
        # 预警日志：信号时间、股票代码、信号类型、价格、触发策略
        self.logger.warning(f'[{signal}预警] {code} 价格 {price} 触发策略: {trigger}')
        # 信号CSV：供其他程序（如自动化交易）读取
        row = pd.DataFrame([{'signal_time': now, 'code': code, 'signal': signal,
