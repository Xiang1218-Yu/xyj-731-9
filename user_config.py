...
#   not     True表示对该因子取反。logic为'AND'/'OR'简写时自动在表达式中生成NOT前缀；
#           logic为自定义表达式时，not只影响评分口径，逻辑取反请在表达式内写NOT
#   enabled 是否启用该因子
# logic 说明：
#   'AND' 全部因子命中才入选；'OR' 任一因子命中即入选；
#   也可写表达式，如 "策略1 AND (策略2 OR NOT 卖策略)"，AND/OR/NOT不区分大小写，
#   表达式是逻辑组合的唯一取反入口（在因子原始命中值上求值）
...
monitor = {
    ...
    'lookback': 3,  # 每轮检测最近N个周期的信号，保证持续运行中不遗漏触发过的信号
    'start_date': '',  # 策略计算起始日期，空=从数据最早日期开始。格式 2020-10-10
    'end_date': '',  # 策略计算截止日期，空=到最新数据。格式 2020-10-10
